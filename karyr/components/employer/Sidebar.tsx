    ...
    <nav className="flex flex-col gap-1">
      {links.map(({ href, icon: Icon, label }) => (
        <Link key={href} href={href} onClick={() => setMobileOpen(false)}
          className={path.startsWith(href) ? 'sidebar-link-active' : 'sidebar-link'}>
          <Icon size={18}/>{label}
        </Link>
      ))}
    </nav>
  ...
  return (
    <>
      <aside className="hidden md:flex flex-col w-60 min-h-screen bg-white border-r border-gray-200 p-4 fixed top-0 left-0 z-30">
        <div className="mb-6 pt-2 px-1"><Logo href="/employer/dashboard" size="md"/></div>
        <p className="px-3 text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Employer Portal</p>
        {nav}
      ...
      <div className="md:hidden flex items-center justify-between px-4 py-3 bg-white border-b border-gray-200 fixed top-0 left-0 right-0 z-40">
        <Logo href="/employer/dashboard" size="sm"/>
