...
const links = [
  ...
  { href: '/college/applications', icon: FileBarChart, label: 'Applications' },
]
  ...
  return (
    <>
      <aside className="hidden md:flex flex-col w-60 min-h-screen bg-white border-r border-gray-200 p-4 fixed top-0 left-0 z-30">
        <div className="mb-6 pt-2 px-1"><Logo href="/college/dashboard" size="md"/></div>
        <p className="px-3 text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">College Portal</p>
        {nav}
        <div className="mt-auto pt-4 border-t border-gray-100">
          <div className="flex items-center gap-3 px-3 py-2 mb-2">
            <div className="w-8 h-8 rounded-full bg-black text-white flex items-center justify-center text-xs font-bold">C</div>
            <div className="min-w-0"><div className="text-sm font-medium text-black truncate">Career Center</div><div className="text-xs text-gray-400">Admin</div></div>
          </div>
      ...
      <div className="md:hidden flex items-center justify-between px-4 py-3 bg-white border-b border-gray-200 fixed top-0 left-0 right-0 z-40">
        <Logo href="/college/dashboard" size="sm"/>
      ...
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-30 bg-black/50" onClick={() => setMobileOpen(false)}>
          <div className="w-64 h-full bg-white p-4 shadow-xl" onClick={e=>e.stopPropagation()}>
            <div className="mb-6"><Logo href="/college/dashboard" size="md"/></div>{nav}
          </div>
        </div>
      )}
    </>
